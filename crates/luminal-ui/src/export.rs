...
use luminal_core::model::{Composition, Document, LayerKind, MatteChannel, ProjectItem};
...
struct Renderer<'a> {
    ...
    gpu: &'a luminal_gpu::GpuContext,
    colour: luminal_gpu::ColourEngine,
    compositor: luminal_gpu::Compositor,
    decoders: HashMap<Uuid, luminal_media::VideoDecoder>,
}
...
impl Renderer<'_> {
    ...
    fn prepare_footage(
        ...
        if !self.decoders.contains_key(&item) {
            let index =
                luminal_media::index::build_frame_index(&info.path).map_err(|e| e.to_string())?;
            let dec =
                luminal_media::VideoDecoder::open(&info.path, index).map_err(|e| e.to_string())?;
    ...
    fn prepare(
        ...
        match &layer.kind {
            ...
            LayerKind::Sequence { clips } => {
                ...
                match luminal_core::sequence::resolve(clips, lt) {
                    Some((_id, luminal_core::sequence::ClipSource::Footage(item), st)) => {
                        use luminal_core::retime::Interpolation;
            ...
            LayerKind::Precomp { comp } => {
                ...
                let mask = (!layer.masks.is_empty()).then(|| {
                    let rgba = mask_rgba(&luminal_core::mask::combined_coverage(
                        &layer.masks,
                        nested.width,
                        nested.height,
    ...
    fn render_comp_linear(
        ...
        let mut draws: Vec<luminal_gpu::CompositeLayer> = Vec::new();
        for l in comp.layers.iter().rev() {
            ...
            let tr = &l.transform;
            draws.push(luminal_gpu::CompositeLayer {
                texture: &p.tex,
                size: p.natural,
                position: (
                ...
                matte: l.matte.as_ref().and_then(|mr| {
                    matte_tex.get(&l.id).map(|mt| luminal_gpu::MatteInput {
                        texture: mt,
...
fn run(
    ...
    let mut renderer = Renderer {
        ...
        items,
        gpu,
        colour: luminal_gpu::ColourEngine::new(gpu),
        compositor: luminal_gpu::Compositor::new(gpu),
    ...
    let mut encoder = luminal_media::Encoder::open_with_bitrate(
        out_path,
        tw,
        th,
...
pub fn camera_mat(
    comp_w: u32,
    comp_h: u32,
    pose: luminal_core::model::CameraPose,
) -> luminal_gpu::Mat4 {
    luminal_gpu::camera_matrix(
        comp_w as f32,
        comp_h as f32,
        pose.zoom as f32,
        (
