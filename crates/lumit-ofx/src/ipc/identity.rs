...
//! # In plain terms
//!
//! The transport lives in `lumit-ipc` and is shared with the audio host and
//! with LFX. These three are what it is *not* shared on: the prefix every OFX
...
/// The endpoint prefix: `lumit-ofx-{identifier}.pipe` on Windows, and the same
/// with `.sock` in the temporary directory elsewhere.
pub const HOST_PREFIX: &str = "lumit-ofx";
