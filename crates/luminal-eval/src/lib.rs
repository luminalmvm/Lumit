...
//!
//! In plain terms: before rendering a frame, Luminal writes down everything
...
use luminal_core::model::{Composition, Document, LayerKind, MatteChannel};
use uuid::Uuid;
...
/// objects defined in luminal-eval itself), so the crate stays engine-pure.
...
fn blend_tag(b: luminal_core::model::BlendMode) -> u8 {
    ...
    match kind {
        ...
        LayerKind::Sequence { clips } => {
            ...
            match luminal_core::sequence::resolve(clips, lt) {
                ...
                Some((_id, luminal_core::sequence::ClipSource::Comp(comp), st)) => {
                    if visited.contains(&comp) {
                        h.update(b"cycle");
                        return Some(());
                    }
...
mod tests {
    ...
    fn content_edits_change_the_key() {
        ...
        c2.layers[0].blend = luminal_core::model::BlendMode::Screen;
        ...
        c5.layers[0]
            .masks
            .push(luminal_core::mask::Mask::rectangle(0.0, 0.0, 10.0, 10.0));
    ...
    fn retime_keys_the_source_frame_not_the_local_frame() {
        ...
        use luminal_core::time::Rational;
        let doc = Document::new();
