...
// a whole-frame tile cut from the frame's middle with no phase (K-442), which
...
//
// **The destination may be BIGGER than the source** (K-442). Output width and
...
struct Params {
    ...
    out_size: vec2<u32>,      // the destination raster, >= the source (K-442)
};
