...
//
// The sampler itself - the shaper, the curve read, the tetrahedral cube read
// and `ocio_apply` - lives in `ocio_sample.wgsl`, which is prepended to this
// file and to the OCIO effects' kernel at pipeline creation, so the Viewer, the
// export and an effect in a stack all read a baked table with one arithmetic.
// This file declares the bindings that sampler reads and the fragment
