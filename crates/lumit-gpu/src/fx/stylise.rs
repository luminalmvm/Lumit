...
pub struct MatteKeyOp {
    ...
    /// Screen pre-blur radius, raster pixels (K-446). 0 skips the stage.
    pub pre_blur: f32,
    /// Screen matte shrink (−) / grow (+), raster pixels (K-446). 0 skips it.
    pub shrink_grow: f32,
    /// Screen matte softness (a blur of the matte), raster pixels (K-446).
    pub softness: f32,
    /// Despot black, 0..1 (K-446).
    pub despot_black: f32,
    /// Despot white, 0..1 (K-446).
...
impl FxEngine {
    ...
    /// Apply one matte key (docs/08 §3.21, K-121/K-154, K-446) to a linear
