//! Matte key (docs/08 §3.21, K-121/K-154, K-446): a Keylight-style
...
//!
//! **The spatial controls** (K-446) are the half of Keylight that cannot be
...
//! `tolerance` / `softness` are superseded and simply go unread. The K-446 rows
...
#[effect(
    ...
    // holds. Since K-446 it is no longer pointwise: the spatial stages judge a
...
pub struct MatteKey {
    ...
    /// (K-446). Grain and compression noise stop reading as detail; the colour
    ...
    /// Keylight's Screen shrink/grow (K-446). Morphological, not a blur: the
    ...
    /// matte, so the picture keeps its own sharpness (K-446). 0, the default,
    /// is the neutral.
    #[slider(
    ...
    /// foreground — is lifted to the darkest of its neighbours (K-446). A pixel
    ...
    /// key made of it (K-446) — the hold-out for a patch of foreground the
    ...
    /// A mask of this layer's whose inside is forced **transparent** (K-446) —
...
impl MatteKey {
    ...
    /// filled in the raster (K-408, K-446). Never a panel row — the same
