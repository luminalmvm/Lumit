...
/// Shake (§3.4). Backed by the reusable [`EdgesMode`] enum (P3), so the
...
/// know — an unknown effect is preserved as an inert placeholder.
///
/// The catalogue also carries whatever registered at run time, so this
/// answers for a scanned OFX plugin as readily as for a built-in.
...
/// What every OFX plugin's `match_name` begins with — the host mints
...
/// What every **VST3 audio plugin's** `match_name` begins with — the
...
pub fn instantiate_for_raster(match_name: &str, w: f64, h: f64) -> Option<EffectInstance> {
    ...
    // The flare's light is px@comp, so its tasteful default — the
    ...
    // centre are all px@comp, and a schema constant that says "960, 540"
    ...
    for p in &mut inst.params {
        let v = match (match_name, p.id.as_str()) {
            ...
            // Planar track's quad (docs/08 §3.87) is the same default
            ...
            // The two search points are px@comp for the same reason,
            ...
            // The Point control is the same default once more: a fresh
            ...
            // Points sample's query point is the same default for the
            ...
            // Radial blur's centre is px@comp as well, so it joins the
            ...
            // and its declared half-a-frame default is only *half a frame* on
            // the comp it landed on. The default Flip order runs left to
            // right, so the width is the axis it is measured along.
            ("card_wipe", "transition_width") => w * 0.5,
            // Tile's centre is the same default for a stronger reason:
            ...
            // The tile's own size and the output window's join it too, for
            // that same reason: they are sizes, so they are px@comp,
...
/// the button kind. A stored value for a button would be a button that
...
pub fn default_param_value(kind: &ParamKind) -> Option<EffectValue> {
    Some(match *kind {
        // A Slider stores as a Float like Int and Angle do — the kind
        ...
        // A fresh curve is the shape its declaration asked for — the
        ...
        // A button holds nothing — see the doc comment above.
        ParamKind::Action => return None,
    })
}

/// Forward-migrate a stack loaded from disk: a built-in instance
...
/// Convert a saved instance's share-of-the-frame values into px@comp,
/// the compatibility read for the move to pixel units.
///
...
pub fn migrate_percent_to_px(effects: &mut [EffectInstance], w: f64, h: f64) {
    for e in effects.iter_mut() {
        ...
        // End. The run is the only basis that was ever right for it, so
        ...
        // measured along whichever axis Flip order runs, so the basis
        ...
        // cents of the frame. Both pairs are sizes, so both convert,
        ...
        // *diagonal*, the last exception to the px@comp rule, so the
        // diagonal is the basis it converts against.
...
/// replaced it. The old parameter had two values:
...
/// layer the effect is being added to (docs/impl/layer-input.md).
///
...
/// visibility half of [`param_enabled`]'s question.
///
...
/// (`build.rs`'s `mattes_for` skips such a row). That is what the Lens
...
pub fn instantiate(match_name: &str) -> Option<EffectInstance> {
    ...
    Some(EffectInstance {
        ...
        // makes Propagate refuse with `NoBaseFrame` rather than guess.
        roto: None,
        extra: serde_json::Map::new(),
    })
}
