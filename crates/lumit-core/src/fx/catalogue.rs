...
//! command palette and the preset browser show, so it is deliberately a
...
crate::catalogue![
    ...
    // The distort batch, appended at the Distortion family's end, in
    ...
    // Wave 2's Distort I batch, appended at the Distortion family's end, in
    // docs/08 §3.48-§3.52 order.
    CornerPinDef => CornerPin,
    DisplacementMapDef => DisplacementMap,
    PolarCoordinatesDef => PolarCoordinates,
    TwirlDef => Twirl,
    SpherizeDef => Spherize,
    // Wave 2's Distort II batch, appended at the Distortion family's end, in
    // docs/08 §3.53-§3.56 order.
    RippleDef => Ripple,
    WaveWarpDef => WaveWarp,
    BezierWarpDef => BezierWarp,
    WarpDef => Warp,
    // The Generate family, in docs/08 §3.34–§3.37 order.
    FillDef => Fill,
    GradientDef => Gradient,
    NoiseDef => Noise,
    FractalNoiseDef => FractalNoise,
    // Wave 2's Draw and grain batch, appended at the Generate family's end,
    // in docs/08 §3.73-§3.77 order. All five draw or lay something over the
    // frame rather than change the colour of what is there, which is what the
    // Generate family is for.
    BeamDef => Beam,
    LightningDef => Lightning,
    RadioWavesDef => RadioWaves,
    VegasDef => Vegas,
    AddGrainDef => AddGrain,
    // The mask carriage's consumers, after them and in docs/08 §3.78-§3.79
    // order: the two effects that draw a mask's own line rather than one found
    // in the picture.
    ScribbleDef => Scribble,
    StrokeDef => Stroke,
    // Particulate, at the Generate family's end: it makes its own pixels
    // rather than changing the ones that arrived, which is what the Generate
    // family is for, and it is the first entry to declare a data output
    // beside its picture.
    ParticulateDef => Particulate,
    // The generators, beside it and for the same reason:
    // they make points rather than pixels, and declare the same Points output.
    GridDef => Grid,
    ScatterDef => Scatter,
    EmitFromImageDef => EmitFromImage,
    // The consumers, after the producers they read: the
    ...
    // Set matte, at the Utility family's end.
    ...
    // Broadcast safe (docs/08 §3.69), after it — a delivery tool rather
    // than a look, which is what Utility is for.
    BroadcastSafeDef => BroadcastSafe,
    // Camera track (docs/08 §3.85), at the Utility family's end. It is a
    ...
    // Planar track (docs/08 §3.87), beside it: the same substrate asked
    ...
    // Roto brush (docs/08 §3.88), after the two tracking handles: it is
    ...
    // Wave 2's Stylise I batch, appended at the Colour family's end, in
    // docs/08 §3.58-§3.63 order. AE files all six under Color Correction and
    // so does Lumit: every one of them is tone or colour maths.
    ...
    // Drop shadow, at the Stylise family's end.
    DropShadowDef => DropShadow,
    // Roughen edges (docs/08 §3.57), at the Stylise family's end after it.
    RoughenEdgesDef => RoughenEdges,
    // Wave 2's Stylise II batch, appended at the Stylise family's end, in
    // docs/08 §3.64-§3.68 order. These five really are stylisations — what
    // each does to a frame is change how it *looks*, not what colour a pixel
    // is — which is what puts them here and Stylise I in Colour.
    MedianDef => Median,
    MosaicDef => Mosaic,
    ...
    // Wave 2's Transitions batch, appended at the Transition family's
    // end, in docs/08 §3.70-§3.72 order.
    VenetianBlindsDef => VenetianBlinds,
    IrisWipeDef => IrisWipe,
    CardWipeDef => CardWipe,
    // The one effect whose program the user writes, appended at the
    ...
    SplitDef => Split,
    CombineDef => Combine,
    // The first driver that reads data rather than only numbers: a Points
    // stream in, a count and a distance out. Last, because it arrived last and
    // the order here is the order the Graph panel's search shows.
    PointsSampleDef => PointsSample,
    // The family's cross-layer tap: the first driver whose *output* is
