...
/// over the in point. Random access is the whole premise here, so this is
...
fn particulate_survives_a_scrub() {
    ...
    // picture whichever way the playhead reached it.
                ...
                Some(want) => assert!(
                    *want == rgba,
                    "frame 90 came back different after scrubbing away and back — a particle \
                     system with no state cannot do that"
                ),
            }
        }
    }
...
/// **The whole claim, through the application**: on a 3D layer the particles
/// are seen through the composition's camera, and on a 2D layer they are not —
/// the camera might as well not be there.
///
/// The second half is the old-project gate at its widest: a project that never
/// asked for depth renders byte for byte whatever the comp is looking with.
#[test]
fn a_three_d_particulate_is_seen_through_the_comps_camera() {
    ...
    assert_eq!(
        ...
        "a camera moved a 2D layer's particles — the old-project guarantee is broken"
    );
