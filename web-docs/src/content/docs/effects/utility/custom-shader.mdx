...
Runs a program you write. **Edit shader…** opens a code editor, or
...
The effect also holds an inner **node graph**: double-click the effect — its box on the
Graph panel or its heading in Effect controls, and it opens the way a precomp does. The
panel draws three kinds of graph on the one canvas: a layer's stack, this inner graph, and
a [node graph composition](/use/node-graphs/).
