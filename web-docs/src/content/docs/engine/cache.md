...
Caches can be cleared by clicking on the cache in Lumits bottom strip, or within settings.

:::note[]
In practice, we actually try to perform a calculation of a frames render time, against
...
When Lumit close, these cached frames do not clear, meaning they can fill up the RAM
