...
1. **The user interface never waits for the engine.** The UI thread never evaluates anything;
   slowness appears as degraded pictures, never as a frozen application.
2. **Degrade, never crash.** Every resource exhaustion path ends in a visible quality
...
Minimum spec is unchanged: Windows 10 20H2+, any DX12-capable GPU, CPU-only operation
functional (slowly) for every built-in effect.

**The reference comp**, used by every budget and built in code by the harness (§7.3),
...
- **B1 and B2 are also the owner's recorded mandate** (2026-08-30), restated as the
...
- B12–B14 are the four numbers that are Particulate's own, and they are per-effect rather
  ...
  time, and the pass checks cancellation between its evaluate and its draw. The fourth
  claim is not a millisecond and so is not a row: under governor pressure the effect
...
- **B15–B17 are the puppet's three**, measured on the *pathological* fixture
  ...
  is arithmetic rather than accident. The **warp** is one bilinear resample and one
  barycentric per pixel in f64; two million of those do not fit 8 ms on one thread, and
  the note's 8 ms is what the GPU warp it already names as the upgrade path is *for*. The
  **solve** at the vertex cap is a forward and back substitution through a
...
- **Where "the reported card memory" comes from**: each platform is asked through
...
- **What is built today**. The "device-epoch object" is the renderer itself: a
  ...
  *is* dropping the renderer. wgpu's device-lost callback raises a flag on the context
  ...
  anything else asks the renderer for anything, and rebuilds on the ordinary build road — the
...
- **Ship the CPU reference implementation**: it is the GPU version's test oracle and
...
per-layer and per-effect indicators) and the rest of it — continuous collection, the
...
of gigabytes, and both times the first question — *is a cache
...
### 7.0.2 Reclaiming what has been dropped
...
- Per-node CPU spans and GPU timestamp queries collected continuously at negligible cost,
  not only in a special mode. **Not what is built:** the shipped measurement fences —
  ...
  *submitted* rather than performed and an unfenced span would time the paperwork. That is a
  ...
  it is never on during playback, and it is **on by default** with the clock in the
  bottom strip as the switch for the session — only the frame the user is waiting on is
  measured. Measuring also gives up the one-command-buffer-per-frame batching
  (§7.0) and hands work over layer by layer, because a fence over a queue that has
  ...
  **the held frame is served at once and composited again, measured, on the next idle
  turn**, so the picture is never made to wait for its own numbers. Timestamp
...
- Timeline column: per-layer render time for the current frame, sortable, with effect-level
  drill-down in a profiler panel ([07-UI-SPEC.md](07-UI-SPEC.md)). **Built:** the
...
**Built.** `crates/lumit-bench` is the harness: a development crate nothing in the
...
- **fp32 comps on the floor machine**: whether fp32 opt-in carries relaxed budgets or
...
- **A stress budget for expressions**: the engine has landed (Rhai) and per-frame
