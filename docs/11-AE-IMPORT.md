...
(docs/impl/ae-import.md §6 phases 1 and 2). **Phase 3 is built too, and it takes both
routes**: File ▸ Import ▸ After Effects project… opens a *file* chooser for
...
a 3D layer's Orientation now does), and §9's row navigation and its persistence in the project
(docs/TODO.md). This document sets the import strategy, and leans on the
AE-compatible keyframe maths and on Retime. Terminology follows
...
|---|---|---|---|
| **Direct `.aep` parsing** (primary) | Nothing — pick the `.aep` | Measured per category against AE's own account of the golden fixture; anything unrecovered is a report row, never a guess | "Imported from After Effects" (+ per-item report rows for anything unrecovered) |
...
The ruling: the user-facing front door is the `.aep` itself — seamless, no script
...
format, and that has not changed: it is the front door because it is seamless and
...
5. **Keyframes, exactly** — per key: time, value, per-side interpolation type (linear /
   ...
   Because Lumit keyframe maths is AE-compatible, this is a value copy, not a
...
`capture.json` is a **faithful, AE-shaped record of the walk** (superseding the
...
(`LIST Als2 ▸ alas`, JSON, `fullpath`); the Bridge route gets it from the DOM.
...
**An image sequence resolves to a frame, not to its folder.** A sequence footage item
...
- **Keyframes** are copied value-for-value: interpolation types, per-side speed and
...
- **3D** maps onto Lumit's 2.5D: 3D layer flags, cameras (both node types),
...
- **Colour**: AE projects are 8/16/32-bpc project-wide with an optional linearised working
  space; Lumit is scene-linear fp16/fp32 per comp. Values convert on import;
...
| Image sequences | mapped | Carried as one footage item ([03-DATA-MODEL.md](03-DATA-MODEL.md) §3.1). AE conforms a sequence by an application *preference* rather than by anything the project file holds, so the rate imports as Lumit's 25 default; the run's length and start come from the folder |
| Comp settings (size, fps, duration, background) | lossless | |
| Layer stack, in/out, start, label, switches | lossless | |
| Transforms + temporal/spatial keyframes | lossless | AE-compatible keyframe maths; includes hold, roving, separated dimensions |
| Parenting | lossless | |
...
| Masks (path, feather, opacity, expansion, modes) | lossless | All seven modes map, **Lighten** and **Darken** included. Two exceptions, both reported: AE's separate x/y feather averages into Lumit's single width, and a **RotoBezier** mask's tangents are computed by AE rather than stored, so it imports as the polygon through its vertices. AE's **variable-width feather** — feather points with positions and radii of their own — is not mapped: Lumit's per-vertex widths are a different anchoring and no fixture proves AE's layout, so the single averaged width stands |
...
| Third-party effects the §5 table names (Sapphire's S_Glow, S_Shake, S_DistortChroma; Lenscare's two defocus effects, …) | **mapped as the plug-in, else nearest** | Two roads (§5a). The user has the vendor's own **OFX build** installed → the effect maps straight to it, matched by plug-in identifier equality, with controls carried by displayed name where the types agree; points are not carried and the report counts what was. No installed build → the **closest Lumit effect at its own defaults**, in the right place in the stack, no dial guessed across the vendor boundary — with one exception, a Sapphire dissolve's Dissolve Percent, which crosses on both roads through the owner's curve. Internals still never map — an equivalent is never claimed, and both roads say what they did |
...
| Expressions | mapped | Imported as source text; run when Lumit's own language can run them ([12-PLUGINS.md](12-PLUGINS.md) §4), else **switched off and listed in the report with their text**, so the keyframes underneath keep driving the property rather than an expression that answers the same wrong number on every frame. The importer decides by trying it: an expression that neither parses nor evaluates against a bare context is After Effects' own language and is not installed |
...
| Cameras, lights, 3D layer flags | mapped | → Lumit 2.5D; renderer-specific shading differs. A camera's Position, **Orientation** and Zoom carry with their keyframes, which is the whole of a tracked camera; depth of field is not read out of the Camera Options group yet, and a **two-node camera's point of interest** is reported rather than approximated — Lumit's camera aims by its own rotation |
...
| Layer styles | mapped | docs/impl/layer-styles.md §7. Each style After Effects reports as **on** becomes a Lumit layer style, in Photoshop's pinned painting order. Angle converts once at the seam — a shadow's light is `270° − a` (AE's 120° default → 150°), an overlay's or a satin's direction `90° − a`; opacity, distances and sizes cross one for one, since the DOM hands per cent and pixels. Boundaries, all reported: a style After Effects lists but leaves **off** is a style nobody added and is not imported (the DOM lists all ten slots on any layer that has ever had the group); **Satin** and **Bevel and emboss** are kept with every value and keyframe but are not drawn in this version (§8); **Pattern Overlay** is not one of the nine and is left behind; the **gradient ramps** After Effects refuses to hand over (Outer glow, Inner glow, Gradient overlay) leave both stops at their defaults; an **outer** style's blend mode other than Normal or Multiply cannot be honoured underneath the layer; `useGlobalAngle` is **baked** into the style's own direction, Lumit having no comp-wide light; per-style `noise`, the glows' Technique, Range and Jitter, and the whole **Blending Options** subgroup are named rather than approximated; and a styled layer that is **rotated or non-uniformly scaled** is told that Lumit runs its styles before the transform where After Effects runs them after (§3) |
...
**The per-parameter correspondence and the unit conversion are code, not rows.**
...
### 5a. Third-party effects have two roads

The rule is that every third-party effect is a placeholder, and its ground still holds: nobody
outside the vendor can re-implement the vendor's internals, so an *equivalent* is never on
offer. What has changed is that Lumit hosts OFX and a scanned plug-in is an
...
offered only where this table has named one. The point of the placeholder rule survives intact:
nothing is ever *silently* something else.

**A Sapphire dissolve is the one dial either road converts rather than copies.** The
...
(the standing precedent), which is exact at every key and an approximation between two keys
...
|---|---|
| "Gaussian Blur" (`ADBE Gaussian Blur 2`) | **Gaussian blur** (built, docs/08 §3.8) — mapped: Blurriness is Lumit's Radius and carries the whole look, AE's raster pixels being Lumit's px@comp (§2.3) — the number is the same. AE's Blur Dimensions (Horizontal / Vertical / both) and its Repeat Edge Pixels have no counterpart — Lumit's Gaussian blurs on both axes and its edge policy is a fixed Repeat — and both are reported |
...
| "Radial Blur" (`ADBE Radial Blur`) | **Radial blur** (built, docs/08 §3.10) — mapped: Amount, Center and the two Types (Spin and Zoom) all have counterparts. No conversions and one report. Amount is pixels on both sides (px@comp, §2.3), and so is **Center** — the two numbers copy across, and nothing is rebased. AE's Antialiasing (Best Quality) and its Random Seed have no counterpart and are reported — Lumit resamples the same way at every quality (§2.3) and its radial blur is not seeded |
...
| "Curves" (`ADBE CurvesCustom`) | **Curves** (built, docs/08 §3.30) — AE's point list is **carried whole** (both sides store a curve as its own control points, so the conversion is a copy of at most sixteen pairs per channel with nothing sampled away). The point list is a `CUSTOM_VALUE` blob AE's own scripting DOM cannot read (the 2026-08-20 audit confirms `ADBE CurvesCustom-0001` is the one property with no readable value), so which route the bundle came by decides the outcome: **the direct `.aep` route decodes the blob and the effect maps** (five channels, Master/Red/Green/Blue/Alpha in that order, each 2–16 points), while **a Bridge bundle still imports the instance as a placeholder** because its capture has the property but no bytes. Two `EffectDiffers` rows on the mapped road name what the points cannot carry: the curve bends scene-linear light where AE bent display values, and the line through the points is drawn with Lumit's clamped ends rather than AE's free ones |
...
| "Brightness & Contrast" (`ADBE Brightness & Contrast 2`) | **Brightness** (built, docs/08 §3.32) — one effect carrying both sliders under AE's names and AE's neutral point |
...
| "Transform" (`ADBE Geometry2`) | **Transform** (built, docs/08 §3.5) — direct on every control that decides the frame: the two points, both scales, **Skew and Skew Axis** (Lumit's pair is After Effects' own shear in After Effects' own order, so both numbers carry unchanged), Rotation and Opacity. AE's Uniform Scale ties Width to Height and hides Width; Lumit has two axes and no switch, so the tie is **resolved** on the way in — the number the person was editing drives both axes — and nothing is lost, so nothing is reported. Its per-effect shutter angle and Sampling have no counterpart and are reported: motion blur is the composition's (docs/08 §2.7) and Lumit resamples at the frame's raster |
| "Motion Tile" (`ADBE Tile`) | **Tile** (built, docs/08 §3.39) — Tile Center, the four Tile/Output sizes, Mirror Edges, Phase and Horizontal Phase Shift all have counterparts. AE keeps the four sizes as per cents of the frame and Lumit as px@comp, so each converts against **the layer's** own extent on the way in — the frame an effect runs on is the layer's raster, not the composition's — and is reported as rebased; both defaults are now the identity, and Output width and height above 100 % grow the layer past its own edges on this side too, so an imported 110 % mirrored tile does what it did in AE |
...
| "Set Channels" (`ADBE Set Channels`) | **Set channels** (built, docs/08 §3.94) — After Effects gives every output channel its own layer picker and Lumit's carriage carries one auxiliary layer per effect, so the four fold onto one: the **first source layer that is neither None nor the layer the effect is on** becomes the Source row, and each output channel then converts exactly when it names that layer (`Source red` and its four siblings), names the layer the effect is on (`Red` and its four siblings — After Effects' own default for a picker nobody touched), or picks Full On or Full Off. A **second, different source layer** and After Effects' **None** are both a picture this conversion will not guess at, so the channel is reported and left at its identity default; the answer is a second copy of the effect. Hue, Lightness and Saturation collapse onto Luminance and say so, exactly as they do everywhere else a channel is picked. Of the reference project's ten instances, eight convert with nothing reported and two carry a None source that does |
| "Channel Blur" (`ADBE Channel Blur`) | **Channel blur** (built, docs/08 §3.45) — mapped: the four radii and "Repeat Edge Pixels" convert directly — AE's radii are raster pixels and Lumit's are px@comp (§2.3), the same number. AE's Blur Dimensions (Horizontal / Vertical / both) has no counterpart — Lumit's is always both — and is reported |
...
| "Beam" (`ADBE Laser`) | **Beam** (built, docs/08 §3.73) — direct: Starting Point, Ending Point, Length, Time, Starting Thickness, Ending Thickness, Softness, Inside Color, Outside Color and Composite on Original all have counterparts under the same names, and Time carries AE's meaning exactly. Three conversions and one report. **Length is a fraction of the run between the two points in AE and px@comp in Lumit**, so it is multiplied by the run those two points describe, read at time zero — a keyframed pair means AE's fraction stood for a distance that moved, and no single pixel number is all of them. **The two thicknesses convert from AE's raster pixels to px@comp** (§2.3). **Softness is measured against the rim rather than against the whole width** (§3.73's third note), so the number carries across as a look rather than as a length. And AE's **3D Perspective** is not carried and is reported: it foreshortens the beam from a camera Lumit keeps on the composition (docs/06), which is §3.72's ruling on Card Wipe's camera in a smaller costume |
...
| "Slider Control" (`ADBE Slider Control`) | **Slider control** (built, docs/08 §3.80) — **pending audit**: direct, the one property onto the one row in the same units, keyframes and expressions included. Nothing is converted and nothing is reported |
...
**Ten rows are marked `pending_audit`** — the five Controls and the five added
with the file itself (Invert, Exposure, Apply Color LUT, Sharpen, Unsharp Mask).
...
**The primary route.** Honest scope: **recover what we can, measure what
...
in the impl note. Nothing is vendored.
...
- The report MUST open automatically after a direct parse, and anything the parser
  could not recover MUST be a row in it (the blanket "structure only" label is retired
  in favour of per-item honesty — the measured recovery earned the front door).
- Anything ambiguous imports as a placeholder or as a static value with a report entry —
  the parser MUST NOT guess silently. **One exception**: inside an effect that is
...
**The surface** (docs/impl/ae-import.md §7 phase C): the user picks the `.aep`
...
- **Refusals are counted per effect instance**: a placeholder for a third-party effect
...
**The reasons are engine data, not engine prose** (docs/17). A row crosses as the
...
- **Loading `.aex` AE plugins — never.** Modern AE plugins depend on SmartFX, AEGP suites,
  ...
  OFX and LFX ([12-PLUGINS.md](12-PLUGINS.md)).
