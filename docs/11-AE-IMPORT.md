...
| Masks (path, feather, opacity, expansion, modes) | lossless | All seven modes map, **Lighten** and **Darken** included (K-445). Two exceptions, both reported: AE's separate x/y feather averages into Lumit's single width, and a **RotoBezier** mask's tangents are computed by AE rather than stored, so it imports as the polygon through its vertices. AE's **variable-width feather** — feather points with positions and radii of their own — is not mapped: Lumit's per-vertex widths (K-445) are a different anchoring and no fixture proves AE's layout, so the single averaged width stands |
...
| "Motion Tile" (`ADBE Tile`) | **Tile** (built, docs/08 §3.39) — direct: Tile Center, the four Tile/Output per cents, Mirror Edges, Phase and Horizontal Phase Shift all have counterparts. both defaults are now the identity (K-442), and Output width and height above 100 % grow the layer past its own edges on this side too, so an imported 110 % mirrored tile does what it did in AE |
