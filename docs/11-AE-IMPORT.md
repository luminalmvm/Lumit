...
| "Transform" (`ADBE Geometry2`) | **Transform** (built, docs/08 §3.5) — direct on every control that decides the frame: the two points, both scales, **Skew and Skew Axis** (K-666: Lumit's pair is After Effects' own shear in After Effects' own order, so both numbers carry unchanged), Rotation and Opacity. AE's Uniform Scale ties Width to Height and hides Width; Lumit has two axes and no switch, so the tie is **resolved** on the way in — the number the person was editing drives both axes — and nothing is lost, so nothing is reported. Its per-effect shutter angle and Sampling have no counterpart and are reported: motion blur is the composition's (docs/08 §2.7) and Lumit resamples at the frame's raster |
