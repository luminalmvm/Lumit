...
Wraps FFmpeg through rsmpeg 0.18. Note the architecture doc's `MediaSource` trait
does not exist yet in code: the seam is the concrete `VideoDecoder` struct plus two
injected closures — `load_or_build_with` (index-cache decision, testable without
FFmpeg) and `pick_first_working` (encoder ladder, testable without hardware).
...
flags, detects variable frame rate, and caches to a `.kidx` sidecar keyed by a blake3
fingerprint (size + mtime + head/tail hash). Stale, corrupt or missing all mean
"rebuild", never an error.

**Seeking** is the performance-critical decision (`decode.rs:193`): seek to the
...
yuv420p so hardware and software share one RGBA conversion path. Skipping the repack
makes preview differ from export by up to 161/255 per byte.
...
The callback (`src/lib.rs:232`) obeys real-time rules: no allocation, no waiting.
...
touching the clock or play state, so solo, mute and move are audible on the next
callback (~10 ms) with no re-bake. Export uses the *baked* mixer (`mix_stereo`);
playback uses the live plan, and the two match sample for sample.
...
byte-deterministic (sorted maps), which matters because the frame cache keys on those
bytes.
...
- **Journal**: every committed `Op` appends one JSONL line with `sync_data`.
  Recovery replays last save + journal, tolerating a torn final line (crash
  mid-append) but stopping at a malformed line mid-file.
...
panel binding over a Global one is a `Shadow`, resolved by precedence and reported
rather than flagged (K-281). Every `ActionId` needs a `description()`, and each
description needs both an `engine_labels.dart` entry and an `app_en.arb` key.
