...
pixel, all at once**, with no shared state between them and no guaranteed order.

Delphi analogy: imagine `for y := 0 to h-1 do for x := 0 to w-1 do` where the body
runs on ten thousand threads simultaneously, each knowing only its own `(x, y)`, and
none allowed to touch another's result during the pass.
...
- **Branches cost.** Threads run in lockstep groups; when they disagree on a branch,
  ...
  are free.
- **Work is dispatched in blocks.** Lumit uses 8×8 blocks, so a 100-pixel-wide image
  dispatches 13 blocks and 4 threads per row fall off the edge. Every kernel must
  check.
...
```wgsl
// crates/lumit-gpu/src/fx_adjust.wgsl:23
...
Every resource is declared with its group and binding number, which must match the
Rust bind-group layout exactly:

```wgsl
// crates/lumit-gpu/src/fx_blur.wgsl:7
struct Params {
...
When it did not, one effect measured 17 920 fp16 ULP of error. Two habits avoid it:
pad explicitly to 16-byte rows, and prefer scalar fields over small arrays —
`fx_hue.wgsl` passes a 3×3 matrix as nine separate floats because a uniform array
strides at 16 bytes while `[f32; 9]` does not.

The same blur file also shows the classic multi-pass structure: `dir` selects the
axis, so one kernel runs twice — horizontal, then vertical — instead of one shader
...
```wgsl
// crates/lumit-gpu/src/fx_gamma.wgsl:41
...
```wgsl
// crates/lumit-gpu/src/composite.wgsl:54
...
first. Both branches are computed; that is normal and usually cheaper than diverging.

Swizzling — `.rgb`, `.xy`, `.zyx` — picks or reorders components. A perceptual
transform reads almost as maths:

```wgsl
// crates/lumit-gpu/src/oklab.wgsl:42
...
```wgsl
// crates/lumit-gpu/src/colour.wgsl:18
@vertex
...
```wgsl
// crates/lumit-gpu/src/scope.wgsl:83
...
```wgsl
// crates/lumit-gpu/src/fx_lens_flare_blur.wgsl:63
...
undefined behaviour, which is why out-of-range threads load zeroes and continue
rather than exiting.
...
```wgsl
// crates/lumit-gpu/src/composite.wgsl:290
...
- Shared helpers such as `bilinear` and `unpremult` are duplicated in each file on
  purpose — there is no include mechanism — and each is annotated with the CPU
  function it mirrors.
...
3. If you touched a `Params` struct, update the Rust `#[repr(C)]` struct in the same
   commit and re-check padding.
4. Run `cargo test -p lumit-gpu -- --test-threads=1`. GPU tests are always serial.
5. `tests/wgsl_validates.rs` catches syntax and validation errors without a GPU, so a
   quick `cargo test -p lumit-gpu wgsl_validates` is the fastest first check.
