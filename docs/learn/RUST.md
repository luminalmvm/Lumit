...
is `Copy` or you borrow with `&`. When the owner goes out of scope the value is
dropped. There is no garbage collector and no manual `Free`.

This is why the codebase clones deliberately, and says so. The edit path clones the
whole document on purpose:

```rust
// crates/lumit-core/src/store.rs:169
...
`Arc<T>` is a shared, reference-counted handle — the closest thing to a Delphi
...
```rust
// crates/lumit-eval/src/lib.rs:114
...
```rust
// crates/lumit-media/src/encode.rs:76
...
encoder ladder, testable without any hardware, because the "does it work" test is
injected.

## 3. Option: there is no null

```rust
// crates/lumit-render/src/source.rs:100
...
```rust
// crates/lumit-core/src/expression/layer.rs:20
fn with_layer<T>(
...
```rust
// crates/lumit-media/src/lib.rs:25
...
```rust
// crates/lumit-bridge/src/api/project.rs:285
...
is an *expression* that produces a value — like Delphi's `IfThen`, but for any type.
...
```rust
// crates/lumit-core/src/anim.rs:353
...
```rust
// crates/lumit-core/src/anim.rs:383
...
well to code, and why the bridge's `op_scope` match is safe: a new `Op` is a compile
error until it is handled.

Variants can carry structured data:

```rust
// crates/lumit-render/src/export.rs:20
...
```rust
// crates/lumit-eval/src/exec.rs:96
...
`&mut dyn CacheStore` is a trait object — dynamic dispatch, like a Delphi interface
variable. The executor takes three of them, so it unit-tests with fakes and no GPU:

```rust
// crates/lumit-eval/src/exec.rs:112
pub fn render_frame(
    graph: &EvalGraph,
    t: f64,
    key: Option<FrameKey>,
...
```rust
// crates/lumit-core/src/time.rs:153
...
```rust
// crates/lumit-core/src/fx/fft.rs:21
...
```rust
// crates/lumit-core/src/time.rs:179
...
ownership of what they capture — necessary when the closure outlives the current
scope:

```rust
// crates/lumit-bridge/src/api/state.rs:432
...
```rust
// crates/lumit-core/src/model.rs:206
...
`zip` walks several collections together — this builds the composite layer list from
five parallel vectors at once:

```rust
// crates/lumit-render/src/realise.rs:679
...
```rust
// crates/lumit-render/src/diskio.rs:246
pub fn spawn() -> DiskIo {
...
Shared counters use atomics — no lock at all:

```rust
// crates/lumit-eval/src/epoch.rs:38
...
**Lock scoping is the house rule.** A lock is held for the shortest possible block,
and never across an await, a GPU call, or a render. The idiom is a block that returns
the data you need:

```rust
// crates/lumit-bridge/src/api/worker_thread.rs:2207
...
`let ... else` is the guard clause: bind on success, or take the `else` branch, which
must diverge (`return`, `break`, `continue`). The guard drops at the closing brace.

Real-time code takes this further — the audio callback never waits at all:

```rust
// crates/lumit-audio/src/lib.rs:232
...
```rust
// crates/lumit-gpu/src/lib.rs:246
...
```rust
// crates/lumit-core/src/time.rs:164
...
```rust
// crates/lumit-media/src/lib.rs:116
    #[test]
...
```rust
// crates/lumit-core/src/time.rs:335
    proptest! {
        #[test]
...
```rust
// crates/lumit-eval/src/schedule.rs:366
...
3. `crates/lumit-core/src/ops.rs` — the command pattern; skim, do not read whole.
...
- `unwrap`, `expect`, `panic!`, `todo!`, `unimplemented!` and `unsafe` are denied
  workspace-wide. Tests re-allow them locally.
