...
`panels/`, `engine/`, `start/` or `reference/`. Most sections are **autogenerated per
directory**, so a new file appears with no config edit. The exception is `use/`,
where the five groups under "Using Lumit" list their pages by hand in
`astro.config.mjs` — a new page there needs a line adding to the group it belongs
to, or it will belong to no topic and the build will say so. Ordering inside an
autogenerated section comes from frontmatter:
...
| Docs sidebar sections | the `starlightSidebarTopics([ ... ])` list in `web-docs/astro.config.mjs` |
...
The docs sidebar is split into three **topics** — Tutorials, Guides and Reference —
shown as a row of links at the top of the sidebar, each with its own list
underneath. That comes from the `starlight-sidebar-topics` plugin, pinned to 0.6.2
because 0.7 and later need a newer Starlight than ours. Every page must sit in
exactly one topic; the build prints a warning for any page that sits in none.

To add a new docs *section*, add a directory under `src/content/docs/`. Then add one
`{ label: "...", autogenerate: { directory: "..." } }` entry to the `items` of
whichever topic it belongs to.

The effect categories under Reference are not typed out. The config reads
`crates/lumit-core/fx-reference.json` when the site builds and takes each
category's real name from it, which is why the sidebar says "Blur & sharpen" and
not "blur-sharpen". Rename a category in the engine and the sidebar follows.
