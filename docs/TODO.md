...
decided; the three surfaces that were only a routing away - the Timeline row's label,
switches and menu, the Effect controls card's `_withHandle`, and the Project panel's
context menu - landed 2026-08-25 and their tests are the record. What is left is the
one that is a model change rather than a loop:
