...
| `lumit-ipc` | The broker transport every plugin host shares: the duplex pipe and its length-prefixed messages, where the broker executable is, and the handful of rules every host must answer the same way. Each host's endpoint prefix and environment variable are passed in rather than baked in, so no two hosts share a namespace. How: [impl/lfx.md](impl/lfx.md) §3.1. |
| `lumit-lfx-abi` | **LFX, the native plugin ABI**: `include/lfx.h` and its `#[repr(C)]` mirror, and nothing else. The one crate in this workspace under **MIT** rather than GPL-3.0-only ([12-PLUGINS.md](12-PLUGINS.md) §3.6), so a proprietary vendor adopts the header without licence anxiety. No behaviour and no dependencies: a helper that drifted in here would become MIT by accident. How: [impl/lfx.md](impl/lfx.md) §2. |
| `lumit-lfx` | **LFX host.** The describe sink and its lowering onto an `EffectSchema`, the catalogue entry a described plugin becomes, the instance pool, the control protocol and the depth-sized ring, bundle discovery and the three tables, `.lfxpack` installation and the trust store - and, in one module, the in-process loader the tests drive the ABI edge with. Shares the broker substrate with `lumit-ofx` through `lumit-ipc`. How: [impl/lfx.md](impl/lfx.md). |
| `lumit-lfx-broker` | The second process an LFX plugin lives in: two arguments and a serve loop over `lumit-lfx`'s own code. Nothing a stranger's module touches runs in the process holding the project. |
| `lumit-lfx-validator` | The shipped **`lfx-validator`** program and the library under it: ten suites driving a bundle through a broker, a markdown table in the OFX conformance bench's shape, and a non-zero exit. What a vendor runs before they ship, and what the `template/` repository's own CI runs. How: [impl/lfx.md](impl/lfx.md) §9. |
| `lumit-lfx-testplug` | The LFX fixture: twelve personalities behind one `lfx_entry_point`, the dangerous three disarmed unless an environment variable says otherwise, and the probes a host's own tests read. A dev-dependency, never shipped. |
