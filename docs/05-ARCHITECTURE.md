# Luminal system architecture

**Status: canonical.** This document defines how Luminal is structured as a codebase and a
...
Luminal is a single cargo workspace of small crates. Small crates keep incremental builds in
...
|---|---|
| `luminal-core` | The document model: project, assets, comps, layers, clips, properties, keyframes, Retime segments, markers. Pure data + command application. No IO, no GPU, no threads. |
| `luminal-time` | Rational time types (`SourceTime`, `ClipTime`, `LayerTime`, `CompTime`, `FrameRate`) and conversions. Depended on by everything. |
| `luminal-eval` | The heart of **Nova** (K-083), the render pipeline: compiles a comp's layer stack into the evaluation graph; scheduling, cancellation epochs, the metadata pass, job orchestration for the pixel pass. |
| `luminal-gpu` | The one wgpu device, WGSL effect kernels, texture pool, readback, device-lost recovery, optional CUDA interop. |
| `luminal-media` | rsmpeg demux/decode/encode, frame index, persistent decoder instances, hardware decode, proxy generation, image sequences. |
| `luminal-audio` | **Pulsar** (K-083): cpal output, audio graph evaluation, the audio clock, peak-pyramid waveform generation, beat detection. |
| `luminal-cache` | **Nebula** (K-083): the three-tier cache (VRAM/RAM/disk), content-hash keys, budget accounting, eviction, the resource governor. |
| `luminal-expr` | QuickJS-ng embedding for expressions (K-063): deterministic runtime, AE-surface library, per-property sandboxing. |
| `luminal-ofx` | OFX host: out-of-process plugin server, C ABI boundary, shared-memory/shared-texture frame transport. |
| `luminal-kfx` | KFX host (K-062). Shares the sandbox/IPC substrate with `luminal-ofx`. |
| `luminal-project` | Serialisation: `.lum` container read/write, operation journal, autosave, relink, migrations. Spec: [10-FILE-FORMAT.md](10-FILE-FORMAT.md). |
| `luminal-ui` | The egui shell: a tiling dock (egui_tiles, K-074) with a bare Viewer, timeline/graph-editor/Viewer widgets, theming per [15-DESIGN.md](15-DESIGN.md). |
| `luminal-app` | The binary: winit event loop, wiring, session lifecycle, crash handler. |

### 1.1 Dependency direction rules

- Dependencies point **downward only**: `luminal-app` → `luminal-ui` → engine crates →
  `luminal-core`/`luminal-time`. No engine crate may depend on `luminal-ui` or on egui, winit, or
  any UI crate. This is the K-012 escape hatch: the UI layer MUST be replaceable (GPUI, Qt
  shell) without touching the engine.
- `luminal-core` and `luminal-time` MUST have no dependency on wgpu, rsmpeg, cpal, or QuickJS.
  The document model is testable on any machine with no GPU and no codecs.
- `luminal-eval` depends on `luminal-core` (reads compiled snapshots) and on `luminal-gpu`,
  `luminal-media`, `luminal-cache` through **trait objects defined in `luminal-eval` itself**
...
- Circular dependencies are a build error by construction; if two crates want each other, the
  shared piece moves down into a new crate or into `luminal-core`.
...
| **Decode threads** | One per active media stream, owned by `luminal-media`, feeding bounded frame queues. Decode never runs on pool workers: long-GOP seeks stall unpredictably and would starve the pool. |
...
`luminal-core` holds the project as an **immutable snapshot + command journal**:
...
On every document edit, `luminal-eval` incrementally recompiles the affected comp:
...
- **One wgpu device** for the whole application (K-011): DX12 backend on Windows, Metal on
  macOS, Vulkan on Windows only when CUDA interop is enabled (see below). `luminal-gpu` owns
...
- **Device-lost recovery** (Windows TDR is routine, not exceptional): all GPU objects belong
  to a device **epoch**. On `DeviceLost`, `luminal-gpu` tears down the epoch, recreates the
...
`luminal-media` wraps rsmpeg behind a `MediaSource` trait (open → probe → indexed frame
...
Luminal is replacing, and "never crashes" cannot be delegated to third parties.
...
Expressions (`luminal-expr`) are in-process but sandboxed per K-063: no IO, no `Date`, no JIT
...
Each failure below is somebody else's decade; each maps to a binding Luminal rule.

| Anti-lesson | What happened | Luminal rule |
|---|---|---|
...
| **Blender's execution-model migrations** | Tiled → full-frame → GPU: three multi-year compositor rewrites to land where it should have started. | Full-frame-per-node on GPU now, ROI/DoD as metadata, tiling only as fallback (§4.1, §5). Execution models are the most expensive thing to retrofit; Luminal picks the endpoint. |
