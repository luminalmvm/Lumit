...
- [ ] A4 DEFERRED (needs the Retime lane pipeline). The Retime row (`speed_property_row`) draws NO lane glyphs — all its editing is via the ◄ ◆ ► navigator, the value box, and the graph editor (the lane deliberately omits it: `timeline/lane.rs` "the Retime channel has no draggable lane glyphs"). Marquee-selecting Time keys needs: (1) draw them as lane glyphs + register in `lane_glyphs`, (2) a Retime-specific lane-drag commit that moves a value key's screen time (reshaping the retime) with the structural [0,dur] endpoints protected, (3) copy support (currently skipped — no paste target). Medium-risk against core Retime editing; do it deliberately (with a repro) in the sweep rather than blind. NOTE: in the GRAPH view the marquee already selects Retime keys (`GraphSelection::retime`).
