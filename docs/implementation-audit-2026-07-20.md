...
| 7.5 | Presets incl. 1440p60 + Master | Partial | 1080p60 / 4K60 / Vertical / Custom only (`export.rs:72-97`) | ◑ Partial · ⚠ can't build lumit-ui here (CI-verified) · added the **YouTube 1440p60** preset (2560×1440 HEVC, VBR 25/35 — matches doc §7.5) across the enum/`ALL`/label/params/filename + the export menu. **Master** left out: it needs DNxHR/ProRes, which the encoder doesn't have (see 7.4) — a codec feature, not a data add |
