...
| 2/3.1 | 48 kHz session rate, RAM ring, lazy decode, background import pass | Partial/deviates | Playback resamples to device rate; whole-file synchronous decode into RAM (`audio.rs:34`); export does use 48 kHz | ◑ **OOM found by owner** (27 GB on a 1 GB film: unbounded decoded-audio retention + re-decode per audio edit) · fixed: byte-budgeted decoded-audio LRU shared by preview + comp mix, re-mix from cache, Settings → Performance budget defaulting to half of system RAM · ⚠ CI-compiled, 👁 re-test with the movie. The full fix (streaming ring, not a whole-comp buffer) is the audio ring box in the dependency graph |
