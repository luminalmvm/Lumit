...
[15-DESIGN-DESK.md](15-DESIGN-DESK.md); this document only applies them.
...
- [ ] **The mark** - twin keyframes, flat colours, at 16 in a 44×28 cell. *drawn*; not built
...
- [ ] **Workspace strip** - lowercase names, the fronted one `text_primary` over a 2px `signal` rule, no fill. `Alt+Shift+1…9`. As built it rides this line under Left and the toolbar strip under Top. *drawn*
- [ ] **Tool options under Left** - the armed tool's options sit on this line beside the workspace words; the strip above the dock is not mounted (§12B.1). *built*
...
- [ ] **Command palette well** - a 200×20 ghost well at the right with the magnifier and the chord as a `text_disabled` hint. *drawn*; not built
...
- [ ] **Tool options panel** - hangs off the rail beside the armed tool while a tool with options is armed: a `surface_1` card, hairline-bounded, one 24px row per option (fill swatch, size well, hardness, opacity, stroke swatch, stroke width, density, expansion, roto size). Selection shows none. *spec*; as built the options sit on the top line (§0.2)
...
- [ ] **A tab group** (two or more panels stacked) - a 24px strip of lowercase tab names; the fronted one `text_primary` over a 2px `signal` rule, the rest `text_muted`; no fill, no outline; a hovered word brightens to `text_primary` alone, with no hover fill or box. *built*
...
- [ ] **Timecode and frame count** - **not on this row** as drawn; the deck's clock is the clock (§12B.5). As built both stay here, because the deck is not always shown. *built*
...
- [ ] **Layer bar** (16 in the row) - `surface_2` with the label colour at 30% (grey room) / 24% (graphite) over it, a 3px solid tab at its start, radius 2, **no name on the bar** unless the setting asks. *drawn*; as built the row is a full-width `surface_2` band with a pixel of ground above and below, the bar on it with the hue mixed in at 30% and outlined in the hue
- [ ] **Selected bar** - 1px `signal` outline. *drawn*; as built the outline takes the accent
- [ ] **Shut layer's keys** - outlined diamonds in `text_secondary` on the bar. *built*
...
- [ ] **Appearance page** - **Grey room / Graphite** only; the theme shelf (Duplicate, Rename…, Delete, Import…, Export…) kept for user overrides; shape row removed; the Scopes and Viewer toggles; Viewer bars arrangement; smoothing. *spec*; as built the two rooms are colour schemes in the row under the Shape chips, Rooms drawn for Desk, and picking Desk changes no scheme
...
6. **Tool options panel** - settled as built: under Left the tools stand on the rail and the options and the workspace words sit on the top line (§12B.1), not hung off the rail and not on a strip above the dock.
