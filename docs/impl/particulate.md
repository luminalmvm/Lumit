...
The effect also hands its particles out as data — the **points stream** (K-446): positions,
...
(K-471) unless noted. Units per docs/08 §2.3; all distances **px@comp** (K-419). Defaults
are chosen to satisfy the "drop it on and it already looks right" rule: gently drifting,
softly glowing motes over the footage.
...
| Size over life | curve (K-412) | unit square | flat 1.0 | Multiplies Size by normalised age. Static in v1, as all curves are. |
...
selection criterion, recorded as K-474, not a styling choice.
...
| Sprite layer | layer reference (K-123) | — | unset | With the standard source combobox (K-142). Rendered once per frame, instanced per particle. **Unset draws discs** — the mode falls back rather than the effect going no-op, because a render mode must always draw something (deviation from the unset-is-identity convention, documented here). |
| Streak length | seconds | 0–0.1 (0+) | 0.02 | Streak mode: a line from `p(t − length)` to `p(t)` — closed form again, no history needed. |
| Max particles | integer | 1–200 000 (1–1 000 000) | 20 000 | **The budget dial** (K-475, §7). Not animatable — it is a capacity declaration, like the flare's ray budget (K-265), and animating a capacity would re-key the governor per frame. |
| Seed | seed | — | 0 | With the standard reseed button (docs/08 §2.4). |

Plus the host-provided Mix row with its Blend choice (K-425) — Add on the Blend combo is
...
`sample_temporally` (K-132) defaults **on**, like every effect: closed form makes
...
keyframed forces — is precisely the simulation this design excludes; recorded in K-474 so
...
Three decisions, each held against K-471's model:

- **Forces are parameters, not nodes.** A force is a per-particle field, and K-471's
...
implementation-time detail of the registry, not a new decision; K-472 already decided the
port exists.
...
- **Export equals preview** (K-031) by construction: no state, no wall clock, no
...
3. **Draw** (GPU raster, instanced): one quad per live particle — feathered disc, sprite
   ...
   (K-425).
4. **CPU reference** (K-019): the same maths into a `Vec`, software dabs for the draw —
...
## 7. Budget and degradation (K-475)

Particulate is a **playback-class** effect, not a member of the physical flare's
owner-set ~2 s-a-frame simulation class (K-353): a particle system that cannot play back
in real time is not a montage tool. Mirroring K-265 — *the budget is the user's dial* —
the dial here is **Max particles**:
...
- **Export** always steps from frame 0 of the run; the sim cache is a preview
  convenience, never an export input (K-031 survives because stepping is itself
  deterministic).

Everything else in this note — closed-form default, the points layout, the cap, the
render modes — is unchanged by the exception existing. Its absence from v1 is K-474.

## 9. Test plan

Landed with the implementation, per K-007. lumit-core unless noted.
