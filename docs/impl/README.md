...
| [audio-effects.md](audio-effects.md) | Built-in audio effects: the Audio category, the rate and the tail through the chain, the log slider, the first suite of fifteen with their DSP and rows, what stays out, and the packages AE1 to AE6 | 09, 12, 07 |
| [audio-nodes.md](audio-nodes.md) | Sound in the node graph: one tap reading filtered to the mix, a layer or a clip, the clip reference kind, Audio level's Source row and its pickers, packages AN1 to AN3 | 09, 10 |
| [audio-timeline.md](audio-timeline.md) | The Audio timeline panel: a row as an audio-only Sequence layer, the clip's fades, its own effect stack and bypass, the stored fade shapes with the overlap as the crossfade's length, the clip chain baked ahead of the layer's, the placement and conversion bridge calls, the panel and its gestures, and the ordered work packages AT1 to AT7 | 09, 07, 03, 06 |
...
| [resource-governor.md](resource-governor.md) | One ledger for the memory a render spends: where the two budgets come from (and what a unified card changes), what is registered against them — the frame's peak, every cache's contents, the decoded frame in flight — the RAII reservation that cannot be released twice or forgotten, which rungs of the degradation ladder the renderer takes on its own and why those two are invisible, the status readout, and what is deliberately left | 13, 06, 05 |
| [node-graph-comp.md](node-graph-comp.md) | The node graph composition: a comp whose picture is nodes and wires instead of layers, the `graph` field and the one guard that keeps a comp one thing or the other, Merge, Switch and Time offset as Compositing entries, the Read node as a synthetic layer, the projection onto the existing driver walk, one draw source and one walker, the Node graph effect and its derived rows, time inside a graph, and the work packages NG1 to NG6 and P1 to P6 | 03, 06, 07, 08 |
...
| [camera.md](camera.md) | The camera: position as the eye, the two-node aim and its Euler composition, the compositor matrix, the tools in the eye model, depth of field by circle of confusion, the 3D views and wireframes, the schema migration, the AE import and the settings dialog arithmetic | 03, 06, 07, 10, 11 |
| [multi-viewer.md](multi-viewer.md) | Several Viewers at once: the view as a thing with an id, where each piece of its state lives, the shared-texture pool collision, scheduling across views on one renderer, the footage and layer views, and the ordered work packages MV1 to MV11 - design only until built | 07, 17, 05, 06, 13 |
| [addons.md](addons.md) | Addons: the model runtime and the model packs the user installs from Settings, what an addon is and is never, the manifest and the install job, where each model runs as a baked analysis with its sidecar and its frame key, the record of which backend produced a frame, and the ordered work packages AD1 to AD6 - design only until built | 12, 07, 04, 08, 10, 13 |
| [lfx.md](lfx.md) | LFX, the native plugin API: the frozen typed C ABI and its parameter kinds lowered onto Lumit's own vocabulary, the manifest read before any of the plugin's code, the depth-sized ring charged to the governor, the host-owned instance pool, `LfxDef` as an ordinary catalogue entry with the placeholder round-trip, discovery and the three tables an Addons page needs, installation and trust on first use, the validator and the template repository, and the ordered work packages - design only until built | 12, 08, 05, 07 |
