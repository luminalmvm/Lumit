...
- `ParamKind::Layer { self_default }` (fx.rs) — declares the effect wants a
  ...
  instance **added to a layer** starts pointed at that layer; the Lens flare's
  Matte row takes it, DoF's does not.
...
- Inspector: a **Layer picker** arm — a dropdown of the comp's layers by name
  (plus "None"), like the Parent picker in the Effect Controls panel.
...
The source combobox does not apply to a this-layer reference (nothing is
...
- **The matte list** carries every effect's Matte row, whatever that effect means by it.
  Depth of field's `depth` and the Lens flare's `matte` moved onto it when they became
...
- **The layer-input list** carries what is not a matte. Four effects take one. Light wrap's
  ...
  **Texture** (docs/08 §3.68) is a surface pressed into the picture; Fast motion
  blur's **Motion vectors** (docs/08 §3.2) is a *field*, a picture whose red and
  green are how far each pixel moved; and Set matte's own source row (docs/08 §3.44)
...
**Which list an effect uses is the schema's own answer, not a table.** The matte
...
- Preview (`build_comp_draws` → `gpu.rs`) and **export** must render the
  referenced layer and thread it **identically** — factor "render layer X alone
  at comp size" into one shared helper both call, as the matte path does.
...
- The resolved bag carries the scalars only — the depth texture is threaded per
  §2, never in the bag; the effect declares that by returning
...
## Reference the layer BEFORE or AFTER its own effects (matte landed)

> **Superseded.** The two-way "after effects" bool below is now a
...
> same `pixels_for`/`prepare` helpers, so preview == export. Storage: the
...
> (echo/flow degrade to a still) is unchanged. The text below is kept for the
> render/key mechanics it pins, which the combobox reuses.
...
**Matte — landed.** `MatteRef` gains `after_effects: bool`,
...
the source's stack the same way, so they match. The frame key folds the
source's stack (shared `feed_effect_stack`) only when the toggle is on.

- This also **corrected a latent preview/export bug**: export was reading the
  matte source's *post-fx* `prepared` texture while preview read source-only,
  so a matte source that happened to have effects diverged. Both are now
  source-only by default; post-fx only when `after_effects` is set.
...
**Depth layer-input — landed.** Rides as a companion
...
Consequences, all documented in docs/08 §3.22:
...
- **Invert** (`depth_invert` bool — the id predates the change that relabelled
  the row and moved it beside the picker). Applied to the depth *after* it is
