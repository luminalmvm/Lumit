...
([08-EFFECTS.md](../08-EFFECTS.md) §3.27). Specs say *what*; this note is
...
renderer keeps (splatted per ray rather than rasterised as quads); *Temporal Glare* [Ritschel et al. 2009] for the starburst maths.
...
`lumit-core` (`lens_files/` + the generated `fx/lens_library.rs`), down from the
original 1299 (a thousand-entry picker is a search problem, not a choice), and
re-verified: every entry must bake a live ghost train AND keep flaring at a three-position
...
`focal / (2·front semi-aperture)` when absent). The **`lens_file` parameter** (the
LUT File pattern) overrides the pick with a user's own `.lens` file: lumit-render
...
is a real multi-layer stack solved by the **characteristic transfer matrix** (which
replaced the single-layer-times-a-quarter approximation): per layer
...
rest of the removal-oriented work when the accuracy programme closed.
...
corner's pupil angle, blended toward the unit circle by Roundness (plus the
wide-open blend — at the native stop the iris retracts behind the circular bore),
feathered by Softness. **Zero-mask corners still trace**: their weight is zero
...
had before four-bounce paths, statement for statement. Then the propagation (shifted by the thin-lens focus term
`f²/(1000·d − f)` mm). Intersection picks the sphere solution closest to the surface vertex. **A ray never
dies at an aperture** — the old skirt clip is gone, and the three ways a walk
...
**The grid side is per PAIR, not per frame.** The Quality ladder
...
else 2.5× (clamped 8..512). The old ½-base rung for tight blobs is gone: a small
...
**The frame-time grid probe.** The bake spread is a bounding box, and the box
...
**The padded flare buffer.** A Squeeze or Scale below 1 makes the combine
sample PAST the base buffer; the zero-outside tap honestly showed nothing there
...
**The frame's dispatch plan.** The GPU sorts combos grid-major, so the table
falls into runs of one grid; `plan_batches` cuts each run into batches of combos and
chunks of lights, and **every batch strides the scratch by its own grid**. Before that,
...
- **`SCRATCH_BYTE_BUDGET` (48 MB) is hard.** The earlier plan's batch size bottomed out
  at one combo and then let eight lights at an Ultra grid ask for ~100 MB anyway. The light
...
- **`STEPS_PER_SUBMIT` (48 M ray–surface steps) cuts the frame into command buffers.**
  ...
  The count covers the trace's ray–surface steps AND the deposit's pixels
  ...
  times the trace's). While only the trace was metered, a frame of
...
## 4. Splatting the ghosts (per-ray, energy-conserving)

**Each ray deposits on its own; rays are never joined.** The renderer used to build
quads out of neighbouring landings and draw those, which is a fair model of a smooth map
and a wrong one at a **caustic fold** — where the map folds back on itself, so a quad
across it spans geometry that is not one patch. Every rescue added along the way
(sub-pixel inflation, sliver parking, the unlit-corner pull-in, vertex-smoothed density)
and the pixel widening with its analytic four-sample coverage existed to survive that
...
1. **The footprint** (`ray_axes`): the image of the ray's pupil cell under this ghost's
   ...
   **longer of the two one-sided differences** (the first version averaged them, and under an
...
2. **The peak** (`splat_ray` up to the divide): flux is `weight × gain × cell_area_px`
   ...
   48 bytes.
3. **The deposit**: a separable quadratic B-spline (§5a-bis) over the
...
- **Caustic density cap (kept unchanged)**: the divisor is floored at
...
**Pass structure.** `trace` then `build_splats`, both dispatched over the batch's
...
**The target is single-sampled and needs no coverage logic** (the earlier measurement
stands, and splatting makes the coverage machinery unnecessary). A tent falls to zero at
the quad's own edge, so there is no silhouette to antialias: no widening, no `dpdx`
barycentric reconstruction, no four-sample test. Bit-stability follows by construction —
fixed instance order, additive blend, one sample. Additively blending fp16 into a 4×
*multisample* target is what was not reproducible run to run on this hardware, and that —
not the draw order, not the bake, not the pools — is what failed the §2.4 assertion for
months; the ~66 MB pooled 4-sample texture and its resolve went with it.

The combine's flare tap is ZERO outside the buffer: squeeze or scale
...
Ghost softness is **px@comp** (a blur radius is a distance, not a share of
...
The blur radius is capped at 80 px and the sum runs through a **workgroup line
cache**: a workgroup covers 64 consecutive pixels along the blur axis, so the
...
- a lens whose ghosts are ALL extreme frame-filling defocus (some process lenses)
  ...
  off-frame, which is why the Projection Optics prescription left the bundle;
...
- a ghost rendered in an extrapolated regime (an f2.8 zoom shot at f1.5) can wear a
  toothed corona at its fold. Grid density (72→288), wavelength count (32→64), the
  pull-in reach, sub-sample inflation, a local branch-jump cull and a 3× feather were
  each ablated in turn, and the corona is invariant to all of them. It is the fold's
  own discontinuous structure; do not re-chase it with guards.
- wide-angle and fisheye prescriptions flare only near a centred light: the angular
  ...
  are bundled (the three-position probe is the curation gate); a user file loads
...
zero from r 0.7 to its edge: its diffraction pedestal ran to the texture
...
**The sprite is baked `STARBURST_FIELDS` (8) times, at eight field angles**:
...
bit-identical to the old one-slice sprite** — a test pins that the blade spikes still count
...
### 5a-bis. The splat reconstruction must partition unity

`ray_axes` returns **half**-axes: the step between neighbouring rays is `2·a1`. The first tent
...
The kernel reaches `±2·a1` and is the **quadratic B-spline** rather than a tent:
a tent is only C0, and the crease at every cell boundary is itself visible on a real,
...
flux is unchanged. `area` and the density cap keep their original meaning in half-axis units. The
...
### 5a-ter. The deposit accumulates in f32
...
addition is not associative, so it broke bit-stability the moment CI ran it. Integer
...
universally available and would make the picture machine-dependent; and restating the
oracle's bound would have recorded the loss rather than fixed it.

### 5a-quater. Big splats deposit into a pyramid
...
is what made Normal slow and Ultra a machine-freezer (the step budget bounds the
submissions; this bounds the work).
...
### 5b. Ghost edges are Fresnel: the knife-edge rim
...
**How fine those fringes are is derivable, and deriving it is the whole of the rim model.** The
...
masks that first shipped had to go**: a single-FFT propagator's output window is `±(N−1)/(4F)`
aperture units and must cover the aperture, so `F ≤ (N−1)/3` — 85 at a 256² transform, and a
4096² one to reach 1000. Their ladder therefore ran `F = 64 … 2`, two to three orders low,
...
the old `i32` slice index used), and the trace WGSL mirrors `fresnel_cs`,
...
### 5c. Coatings are chosen per glass element

The transfer matrix (§1) made a surface's reflectance a real multi-layer stack. What
follows makes the *choice of stack* the user's, per element.
...
byte-for-byte the pre-palette picture), uncoated, and five real designs whose residual
...
The **auto-exposure gain** closes the loop: the bake renders the CPU reference
...
at the lens's NATIVE stop**, never the working one: the gain is a property of the
...
**64**: a wash-only lens has almost no probe energy, and an unbounded loop would
...
the exposure probe's trace is roughly 0.5 s and the starburst 0.12 s (the eight field
...
**it now runs on a bake thread beside the frame** — see §5a. Three economies,
...
- spreads are measured **after** the ranking and only for the first `MAX_RENDERED_PAIRS`
  ...
  directions — on-axis and a representative off-axis beam — and takes the larger
...
### 5d. The bake key, and what stays per-frame
...
the blade count, and the four continuous iris dials — working f-stop, aperture
...
**What the working f-number is still doing in the key** (worth asking, since the exposure
gain no longer reads it): one thing, the starburst. `bake_aperture_field` takes
`effective_roundness(p.roundness, p.fstop, native_fstop)`, so the wide-open blend —
...
**A provisional frame is named, and then checked.** A frame that fell back to the previous
lens is still a frame nobody may bank — the tiers are keyed by what is *in* a frame — but
that is now decided by *counting the fallbacks*, not by "is any bake in flight?".
`FxEngine::flare_substitutions` bumps at the one place `baked()` hands back other optics
than the key names; callers read it either side of a render and drop the name only if it
moved. `frame_key` names every frame. The old rule took the whole project down with one
animated dial.

**A file parameter's key covers the file itself**: `lumit-eval` folds a `.lens` (or
...
### 5a. The bake runs beside the frame
...
- **Exact by default.** Deferring is a per-engine switch that is *off* until something
  ...
  bit-for-bit what it was (preview-equals-export is untouched).
- **A provisional frame is not banked** (replacing "is unnameable"). A frame drawn
  with the previous lens and filed under the new lens's name is an entry that lies about
  its own content, and the tiers are keyed by content — nothing later would ever
...
The GPU side caches uploaded bakes by key, **evicting oldest-first at 24**.
Emptying the map at the cap, as it used to, made trying lenses quadratic: every ninth pick
...
## 6. Matte source mode (shipped)

Shipped as the **Matte** source mode (docs/08 §3.27): the flare
...
(`MAX_SOURCES`, raised from 8 to 16) with a 2-tile Chebyshev non-max suppression, each gated
by the soft Threshold. **The gate is one-sided**: closed at and below the
...
**Each tile carries the statistics of its whole lit area, not one pixel** — the
...
**Area sources are integrated per ray, not replicated.** Each anchor's second moments
give its half-extent — the standard deviation of its flux about its centre — and that
half-extent travels with the light, one slot per source, into the trace. The flare of an
extended source *is* the integral of the point flares across it, and the trace already
integrates over the pupil, so the two integrals are done together: the ray at pupil-grid
(i, j) offsets its light position inside the ±extent rectangle by `source_jitter` and
computes its own direction from there, carrying the light's **full** colour (no flux
shares — the pupil grid averages). An area source therefore costs exactly what a point
source costs, whatever its size.
...
`PHI_U` is 1/ρ of the plastic constant, `PHI_V` is 1/ψ of the supergolden ratio,
...
- **Each constant must be a good rotation alone**, because each drives its own
  axis by its own index. The first version took the plastic constant's 2D pair
  (1/ρ, 1/ρ²), whose second number is within 0.002 of 4/7 — as a standalone 1D rotation
  its samples fall into seven combs that drift too slowly to wash out across a pupil
  grid, and every area source wore them as stripes. 1/ψ is the same family of cubic
  Pisot units, rationally independent of 1/ρ, and measured cleanest of a scanned battery
  (`lens_flare_an_area_source_renders_without_stripes` pins the metric).
- **Each band re-samples the source at its own phase.** Bands trace and splat
...
The old replication — up to `AREA_SAMPLES_MAX²` = 25 point lights per source, expanded on
...
offsets are a pure function of the ray's own grid indices, so bit-stability survives.
**Area sources: every gated tile's flux then accumulates onto its nearest anchor** — per
tile, `(use source ? the tile's brightest pixel's RGB : white) × the gate`, nearest by
Chebyshev with ties to the lowest anchor index, tiles visited in index order so the float
sum matches the CPU reference op-for-op — and each anchor is written as a light: position
at its own source pixel, colour = the summed flux × the Light tint. A one-tile point
source is its own anchor's only contributor and reads exactly as before; a practical
spanning many tiles finally weighs as its whole lit area instead of one pixel. Every
downstream stage runs per light on the dispatch z axis — the trace
...
`1/(nx·ny)` of the light each, which is the shift-invariant convolution of the
...
spike. The field slice and azimuth are worked out **per stamp**, so a smeared
...
### 6a. Which layer the matte reads
...
Pointing the parameter at any other layer keeps the original behaviour unchanged (rendered
alone at this raster, its own masks and effects per the source mode). The same rule
covers every layer-input parameter, not just this one.

### 6b. Blending the element in
...
- **Sub-pixel deposits are silently dropped by every rasteriser.** The caustic flux that
  ...
  load-bearing; measured without its predecessor (the old quad inflation), the frame's
  dynamic range collapsed from ~116× to 6.6×.
- **…and rescuing a sub-pixel SLIVER by scaling it draws a streak.** This is the trap
  splatting removed rather than guarded: a *quad* straddling a fold has near-zero area and
  large extent, so scaling it about its centroid to reach a px² floor multiplies its
  *length* by up to 100× — a 20 px sliver becomes a 2000 px line, which shipped
...
- **A hard boundary anywhere in the walk becomes a staircase.** The pupil grid samples
  ...
  TIR — draws its edge as blocks the size of a cell. Every one was removed: geometry
...
- **A ray must deposit over its FOOTPRINT, never as a point.** The Monte-Carlo variant
  of this model (tried first) splatted each ray as a point and needed orders of
  magnitude more rays for the same smooth rim. The splat here is not that: each ray spreads
...
- **The exposure loop amplifies whatever survives rendering.** Any attempt to fade an
  ...
  probe can see.
- **Roundness must be an SDF lerp to the circle, not an additive bulge**: the
...
- **An over-long submission costs the DEVICE, not a frame.** macOS and Windows both kill
  ...
  eight matte sources) must break its frame into bounded submissions. This is the
...
2. **Optics units**: Cauchy reproduces (n_d, V); Snell at 45°; TIR returns None;
   normal-incidence Fresnel = ((n1−n2)/(n1+n2))². Coatings on n = 1.5 crown: a
...
4a. **Four-bounce paths**: the sentinel form of a bright pair traces to a finite
...
4b. **Splat guard**: an ordinary footprint deposits exactly the flux put into it
...
4c. **Grid budget**: `pair_grid` is monotonic in spread, clamped to
   ...
   raw-rows seam entry.
4d. **Per-ray source integration**: a point source's jitter is exactly zero over a
   ...
   the same source rebuilt as the old 5×5 replication has strictly more
...
5. **GPU trace oracle** (§8.5 shape): corner-for-corner against
...
6b. **Ghost blur**: the blur alone against the CPU reference on a frame large
...
6c. **Frame plan (no GPU needed)**: every (combo × light) is dispatched exactly
...
6d. **Padded anamorphic**: at squeeze 0.5 the buffer doubles in width, the
...
7. **Matte mode**: GPU detection + per-light flare against the CPU reference at the
   frame bound; the shared MAX_SOURCES / DETECT_TILE constants pinned. Area flux: a
   multi-tile disc's anchor carries several times a one-pixel dot's flux, the dot reads
   exactly as the point always did, both anchors sit on their sources
   (`lens_flare_detects_area_sources_as_summed_flux`).
7b. **Custom lens file**: a bundled lens's text fed through `bake_with` as a
...
7c. **An animated aperture caches**: two f-stops inside one step key the same
...
7d. **The exposure is the lens's, not the iris's**: two working f-stops on one
...
8. **Neutrals and blend**: Normal shows the element alone on opaque black; Add
...
8b. **This layer**: a fresh flare's Matte points at the layer it was added
...
10. **Shader validity** (`lumit-gpu/tests/wgsl_validates.rs`): every `.wgsl` in
...
12. **Eyes** (`lens_flare_dump_frame`, `#[ignore]`d): renders one frame through the real
   ...
   gain overrides). The artefact work was driven by looking at these — no numeric
