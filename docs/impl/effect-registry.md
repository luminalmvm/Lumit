...
Each line names the effect's behaviour (its `EffectDef`) and the parameter block
`#[derive(Effect)]` generated its declaration from. It expands to `pub static
BUILTIN_DEFS: Catalogue` — the definitions the frame walk dispatches through — and `pub
const BUILTINS: &[EffectSchema]` (unchanged in type, so nothing downstream moved), which
is the same 35 declarations in the same order, so the two halves cannot disagree about
which effects exist. The order is the source order, so the Add-effect menu is stable;
there is no start-up cost and nothing runs before `main`.
...
4. **Landed.** `Resolved` and the hand-written `BUILTINS` body are deleted (`resolve_one`,
   ...
   `ResolvedStack::rescale_spatial`, which it only ever delegated to. `run_ops` and
   `cpu::apply_stack` walk `stack.iter()` and match on nothing at all; the aux counters
   advance in the same order they did, and `resolve_stack_temporal_named` still reports its
   id list 1:1 with the ops. `BUILTINS` is now generated by `catalogue!` beside
   `BUILTIN_DEFS` (§2.6), and test 5 below went with the literal it compared against.
...
4. `every_parameter_declares_a_unit`, `only_spatial_values_rescale`,
   ...
   drives the real `ResolvedStack::rescale_spatial` on a resolved blur and pins the radius the old
...
5. ~~`the_generated_schema_matches_the_hand_written_one`~~ — the migration-only test, held
   each generated `Effect::SCHEMA` against the `BUILTINS` literal it replaced, field for
   field. That is what made the port mechanical rather than a rewrite; it was deleted with
   the literal in step 4, when there was nothing left to compare against.
