# Audio plugin hosting: CLAP and VST3

**Built so far: AP1, AP2, AP3, AP4 and AP5 —
...
**Two things AP3 settled differently from the words below, both marked where they
occur:**
...
- **VST3** (Steinberg): dual-licensed — proprietary Steinberg agreement *or* **GPLv3** —
  ...
  *separate objects* the host must wire together. Bind the **flat** vtables, never the C++
  ...
  > **What AP4 bound instead.** The `vst3` crate (MIT/Apache-2.0, coupler-rs) —
...
- **VST2 is dead for us**: Steinberg stopped granting licences in 2018; there is no
  GPLv3-compatible road. Not hosted, not planned.
...
That last sentence is now a fact about the code rather than an intention.
...
place: `EffectDef::open_audio`, which every built-in answers `None`. That is also
...
stack indices, so a drag within the group is a reorder of the chain; the
Graph panel shows it as a node (mint audio family). An audio effect on a layer
with no sound is inert with the same calm the visibility switch rules use.
...
The realtime callback (docs/09 §3.1) must never wait on another process. So
...
> **What AP3 built instead.** No worker thread and no ring: the chain is run over
...
would swell 3 dB mid-ramp and would even alter a passthrough plugin's dry block) —
...
same one OFX reads. Dry over silence because in a montage the music continuing
...
row `p1234`. It is not pretty and it is not the parameter's name, because the name
...
and that is what the switched-off list holds and a saved project stores. Neither
...
`IUnitInfo`: the rows read fine flat, and a whole extra COM interface for twirl
...
start (~10.7 ms control rate, the same rate the Volume envelope already uses) —
...
> **VST3 has no `flush`, so the baseline rides too.** CLAP can hand a plugin a
...
four bytes of length and then the two runs, and loaded into **both** halves.
Never parsed, always round-tripped, exactly like OFX custom parameters (docs/12 §2.2) under the
...
- Watchdog: **per-block deadline = the lookahead margin** (§3), not OFX's 10 s; control
  actions (activate, state) keep the 2 s; describe waits under the ten-second handshake ceiling, because the first one opens the module from disk. Three strikes → session disable +
...
not broken. Written down here so nobody discovers the gap in a release note.
...
1. **In-tree test plugin** (`lumit-aplug-testplug`, mirroring `lumit-ofx-testplug`): a
   ...
   exports `GetPluginFactory` as well**, so the eight personalities can be laid
...
2. **Order of actions** pinned verbatim like `RENDER_ACTIONS`: factory →
   ...
   **Two lists, not one**: `HOST_ACTIONS` and `VST3_HOST_ACTIONS`, because the
...
3. **Chain seam**: empty chain leaves the decoded buffer untouched (pointer-equal);
   ...
   baked mixdown, sample for sample (the preview==export shape); insert runs
...
| AP2 | Broker isolation ✅ | `lumit-aplug-broker`: pipe + block ring + watchdog + disable list + quirks table + brokered scan; plan 5 |
| AP3 | Mix-graph seam ✅ | `EffectDef::open_audio` + `fx::audio_chain`: whole-span bake (no worker or ring), dry fallback with a linear splice, latency shift, per-block param envelopes read through the driver walk, `plugin_state` in the `.lum`, offline bake, CLAP registered from the bridge; plans 3, 4 |
| AP4 | VST3 host ✅ | `lumit-aplug::vst3` + `abi`: bundle layout, factory, component/controller wiring, stereo bus arrangement, normalised/plain at the boundary, per-block queues, two-blob state, through the same broker binary and the same seam; the in-tree fixture wears both faces; plans 1, 2, 6, 7 for VST3 and plan 5 through `run_chain`. Plans 8 and 9 — the conformance bench and the manual pass against real plugins — still owed |
| AP5 | Panel surface ✅ | One Audio plugins group in Effects & presets and the Add effect menu (namespace `audio`, heading worded by the frontend), provenance + switch-off in the row's menu; the rack under an Audio heading at the foot of Effect controls' stack, ordinary cards at their stack indices; the calm badge fed by the chain bake (per-link dry blocks → the OFX errored table) and by the switched-off list; the switch folded into the mix signature so flicking it re-bakes; the Mixer strip's FX chip; VST3 rows stay top-level (no `IUnitInfo`); GUI window recorded as the follow-on |
